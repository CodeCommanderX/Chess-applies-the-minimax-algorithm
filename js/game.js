...
com.khoitao = function (stype){
	com.nowStype= stype || com.getCookie("stype") ||"stypeX";
	var stype = com.stype[com.nowStype];
	com.width			=	stype.width;		//Chiều rộng
	com.height			=	stype.height; 		//Chiều cao
	com.spaceX			=	stype.spaceX;		//Trục X
	com.spaceY			=	stype.spaceY;		//Trục Y
	com.pointStartX		=	stype.pointStartX;	//Tọa độ X của điểm va chạm đầu tiên;
	com.pointStartY		=	stype.pointStartY;	//Tọa độ Y của điểm va chạm đầu tiên;
	com.page			=	stype.page;			//Thư mục ảnh
	com.canvas			=	document.getElementById("chess"); //vẽ
	com.ct				=	com.canvas.getContext("2d") ;
	com.canvas.width	=	com.width;
	com.canvas.height	=	com.height;
	com.childList		=	com.childList||[];
	com.taiAnh(com.page);//Tải hình ảnh
}
...
window.onload = function(e)
{
	...
	com.mans	 ={};		//Bộ sưu tập quân cờ
	//Thoát menu ở ngoài
	com.get("menuQuit").addEventListener("click", function(e) {
		com.get("indexBox").style.display = "block";
		com.get("menuPC").style.display = "none";
	})
	//Người với máy
	com.get("computervsHuman").addEventListener("click", function(e) {
		com.get("indexBox").style.display = "none";
		com.get("menuPC").style.display = "block";
	})
	//Người với người
	com.get("humanvshuman").addEventListener("click", function(e) {
		location.href = "./human.html";
	})
	//bắt đầu chơi
	com.get("playBtn").addEventListener("click", function(e) {
		play.isPlay=true ;
		var dosau = parseInt(getRadioValue("dosau"), 10) || 5;
		play.khoitao( dosau );
		com.get("chessBox").style.display = "block";
		com.get("menuBox").style.display = "none";
	})

	//Nhận giá trị đã chọn của nút radio
	...
	com.get("restartBtn").addEventListener("click", function(e) {
		if (confirm("Bạn có chắc chắn muốn bắt đầu lại không ?")){
			play.isPlay=true ;
			play.khoitao( play.dosau,play.nowMap );
...
//Hiển thị phác thảo cầm đồ di chuyển
...
//com.alert Viết tắt cho , coi tên biến z là tên ít được sử dụng nhất
var z = com.alert;
var l = console.log;

//Lấy khoảng cách của phần tử từ phía bên trái của trang
...
com.createMove = function (map,x,y,newX,newY){
	...
	delete map[y][x];
	if (man.my===1){
		var mumTo=["Một","Hai","Ba","Bốn","Năm","Sáu","Bảy","Tám","Chín","Mười"];
		newX=8-newX;
		h+= mumTo[8-x];
		if (newY > y) {
			h+= " di chuyển tiến ";
			...
			h+= " di chuyển sang ngang ";
			h+= mumTo[newX];
		}
	}else{
		var mumTo=[" vị trí １"," vị trí ２"," vị trí ３"," vị trí ４"," vị trí ５"," vị trí ６"," vị trí ７"," vị trí ８"," vị trí ９"," vị trí 10"];
		h+= mumTo[x];
		if (newY > y) {
			h+= " di chuyển tiến ";
			if (man.pater == "M" || man.pater == "S" || man.pater == "X"){
...
com.luatchoi ={}
//Xe
com.luatchoi.c = function (x,y,map,my){
	var d=[];
	//tìm kiếm bên trái
	for (var i=x-1; i>= 0; i--){
		if (map[y][i]) {
...
//Ngựa
com.luatchoi.m = function (x,y,map,my){
	var d=[];
...
//Tượng
com.luatchoi.x = function (x,y,map,my){
	var d=[];
	if (my===1){
...
//Sĩ
com.luatchoi.s = function (x,y,map,my){
	var d=[];
	if (my===1){ 
...
//Tướng
com.luatchoi.j = function (x,y,map,my){
	var d=[];
...
//Pháo
com.luatchoi.p = function (x,y,map,my){
	var d=[];
	var n=0;
	for (var i=x-1; i>= 0; i--){
		if (map[y][i]) {
...
//Tốt
com.luatchoi.z = function (x,y,map,my){
	var d=[];
	if (my===1){
		if ( y-1>= 0 && (!com.mans[map[y-1][x]] || com.mans[map[y-1][x]].my!=my)) d.push([x,y-1]);
...
//Quân đen quay lui là sự đảo ngược vị trí giá trị của chữ màu đỏ
...
//Quân cờ
com.args={
	//Tên quân/ảnh/vị trí/địa chỉ
	'c':{text:"Quân xe", img:'r_c', my:1 ,bl:"c", value:com.value.c},
	'm':{text:"Quân mã", img:'r_m', my:1 ,bl:"m", value:com.value.m},
	'x':{text:"Quân tượng", img:'r_x', my:1 ,bl:"x", value:com.value.x},
	's':{text:"Quân sĩ", img:'r_s', my:1 ,bl:"s", value:com.value.s},
	'j':{text:"Quân tướng", img:'r_j', my:1 ,bl:"j", value:com.value.j},
	'p':{text:"Quân pháo", img:'r_p', my:1 ,bl:"p", value:com.value.p},
	'z':{text:"Quân tốt", img:'r_z', my:1 ,bl:"z", value:com.value.z},
	
	//tương tự
	'C':{text:"Quân xe", img:'b_c', my:-1 ,bl:"c", value:com.value.C},
	'M':{text:"Quân mã", img:'b_m', my:-1 ,bl:"m", value:com.value.M},
	'X':{text:"Quân tượng", img:'b_x', my:-1 ,bl:"x", value:com.value.X},
	'S':{text:"Quân sĩ", img:'b_s', my:-1 ,bl:"s", value:com.value.S},
	'J':{text:"Quân tướng", img:'b_j', my:-1 ,bl:"j", value:com.value.J},
	'P':{text:"Quân pháo", img:'b_p', my:-1 ,bl:"p", value:com.value.P},
	'Z':{text:"Quân tốt", img:'b_z', my:-1 ,bl:"z", value:com.value.Z}
};

com.class = com.class || {} //thể loại
com.class.Man = function (key, x, y){
	...
	this.bl = function (map){
		...
		return com.luatchoi[o.bl](this.x,this.y,map,this.my)
	}
}
