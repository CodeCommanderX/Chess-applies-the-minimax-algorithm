...
function setNextTurn(){
    ...
    showTurn.textContent =  `Vòng ${Math.floor(counter/4)} : Lượt đi ${currentTurn} di chuyển`  // 2 clicks for both red and black valid moves, so 4 clicks a turn
}

const square = Array.from(document.querySelectorAll('.row .square'));
// removing all the color red squares on board
...
function createPiece(name, color, row, column) {
    ...
    img.setAttribute('src', `img/stype/${color}_${name}.svg`)  //link the image
...
function printGameRecord()
{
    ...
    const genStraightLineMove = piece => 
    {
        if(currentPiece.color == 'red'){
            ...
            if(currentPiece.row < destination.row){      // forward for red
                move[Math.floor(counter/4) - 1].push([`Quân ${piece.toUpperCase()} red vị trí ${10 - currentPiece.column} di chuyển tiến ${Math.abs(parseInt(destination.row)-currentPiece.row)} nước đi`]) ;
            }else if(currentPiece.row > destination.row){ // backward for red
                move[Math.floor(counter/4) - 1].push([`Quân ${piece.toUpperCase()} red vị trí ${10 - currentPiece.column} di chuyển lùi ${Math.abs(parseInt(destination.row)-currentPiece.row)} nước đi`]) ;
            }else if(currentPiece.row == destination.row){
                move[Math.floor(counter/4) - 1].push([`Quân ${piece.toUpperCase()} red vị trí ${10 - currentPiece.column} di chuyển ngang ${10 - destination.column} nước đi`]);
            }
        }else{
            if(currentPiece.row < destination.row){      // backward for black
                move[Math.floor(counter/4) - 1].push([`Quân ${piece} black vị trí ${currentPiece.column} di chuyển lùi ${Math.abs(parseInt(destination.row)-currentPiece.row)} nước đi`]) ;
            }else if(currentPiece.row > destination.row){ // forward for black
                move[Math.floor(counter/4) - 1].push([`Quân ${piece} black vị trí ${currentPiece.column} di chuyển tiến ${Math.abs(parseInt(destination.row)-currentPiece.row)} nước đi`]) ;
            }else if(currentPiece.row == destination.row){
                move[Math.floor(counter/4) - 1].push([`Quân ${piece} black vị trí ${currentPiece.column} di chuyển ngang ${destination.column} nước đi`]);
            }
        }
    };
    const genDiagonalMove = piece => 
    {
        if(currentPiece.color == 'red'){
            move[Math.floor(counter/4) - 1] = [];
            if(currentPiece.row < destination.row){      // forward for red
                move[Math.floor(counter/4) - 1].push([`Quân ${piece.toUpperCase()} red ${10 - currentPiece.column} di chuyển tiến ${10 - destination.column} nước đi`]) ;
            }else if(currentPiece.row > destination.row){ // backward for red
                move[Math.floor(counter/4) - 1].push([`Quân ${piece.toUpperCase()} red ${10 - currentPiece.column} di chuyển lùi ${10 - destination.column} nước đi`]) ;
            }
        }else{
            if(currentPiece.row < destination.row){      // backward for black
                move[Math.floor(counter/4) - 1].push([`Quân ${piece} black ${currentPiece.column} di chuyển lùi ${destination.column} nước đi`]) ;
            }else if(currentPiece.row > destination.row){ // forward for black
                move[Math.floor(counter/4) - 1].push([`Quân ${piece} black ${currentPiece.column} di chuyển tiến ${destination.column} nước đi`]) ;
            }
        }
    };
